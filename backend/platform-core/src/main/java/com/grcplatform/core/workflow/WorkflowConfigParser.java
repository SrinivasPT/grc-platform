 ...
 *
 * Security: Jackson is configured with polymorphic typing DISABLED (OWASP A08).
 * No @JsonTypeInfo or type coercion; strict model classes only.
 */
public class WorkflowConfigParser {
    ...
    public WorkflowConfig parse(String json) {
        ...
        try {
            ...
            throw new ValidationException("config", "workflow config is not valid JSON: " + e.getMessage());
        }
        validate(config);
        return config;
    }
    ...
    private void validate(WorkflowConfig config) {
        if (config.initialState() == null || config.initialState().isBlank()) {
            throw new ValidationException("initialState", "workflow config must specify initialState");
        }
        if (config.states() == null || config.states().isEmpty()) {
            throw new ValidationException("states", "workflow config must define at least one state");
        }
        if (config.states().size() > MAX_STATES) {
            throw new ValidationException("states", "workflow config exceeds maximum of " + MAX_STATES + " states");
        }
        if (config.transitions() != null && config.transitions().size() > MAX_TRANSITIONS) {
            throw new ValidationException("transitions", "workflow config exceeds maximum of " + MAX_TRANSITIONS + " transitions");
        }
        boolean initialFound = config.states().stream()
                .anyMatch(s -> s.key().equals(config.initialState()));
        if (!initialFound) {
